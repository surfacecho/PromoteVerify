...
class Worker(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            if command == 'stop':
                Logger.getlogger().info("Thread: %s, exit via command='stop' flag!", threading.currentThread().getName())
                break
            try:
                if command != 'process':
    ...
    def dismiss(self):
        command = 'stop'
        Logger.getlogger().info("Thread: %s, call dismiss(), set command: 'stop'!", threading.currentThread().getName())
        self.in_queue.set((command, None, None, None))

    def report_error(self):
        '''We "report" errors by adding error information to err_queue.'''
        self.err_queue.set(sys.exc_info()[:2])
...
class ThreadPool():
    ...
    def destroy(self):
        Logger.getlogger().info("Thread: %s, call destroy(), then call dismiss(), join()!", threading.currentThread().getName())
